...
3. **Docs in the same PR:** tick the finished `docs/ROADMAP.md` items; update `CLAUDE.md` Architecture if the structure changed, `docs/privacy-policy.html` (both languages) if user data, permissions, or sharing changed, and `docs/RELEASING.md` if setup changed. Every English message has its translations.
