...
- **QTY-5** Display:
  ...
  - Arabic digits take the Arabic decimal mark: ١٫٥ كغ, never ١.٥ (LANG-3, 25 September 2026).
...
- **PLAN-1** The plan shows one week: a strip of its 7 days at the top and, under it, the chosen day's four meal slots in this order: فطور، غداء، عشاء، وجبة خفيفة (breakfast, lunch, dinner, snack). A day in the strip that has entries carries a dot (amended 26 September 2026, Decision 23; the week used to be one long list of 7 days).
  ...
  - It opens on the current week with today chosen and highlighted. Arrows move a week at a time and keep the same weekday chosen; "هذا الأسبوع" comes back to today.
  - An empty slot shows only its name and an add control.
...
- **PLAN-5** "Add to groceries" in the plan lists the recipe entries of the week shown, from today on, each ticked, so the user can untick what's already at home.
...
- A recipe app is judged at first sight by its food. Photo-forward layouts — big photo cards, a hero photo under the recipe, a floating bar — are what people now expect from one (26 September 2026, from recipe-app work on Dribbble and Behance, taken as ideas, never copied). They are all drawn as if every recipe had a beautiful photo; ours have to look finished when one doesn't, since a typed or pasted recipe usually has none.

- **LOOK-1** One design, **سُفرة / Sufra** (Decision 23), in two accent colours picked in Settings under "الطراز / Look": **زعفران / Saffron**, a paprika orange and the default for a new install, and **حبر / Ink**, a teal. An install that already stored a choice keeps it. The accent is independent of the light/dark setting ("المظهر"), so there are four combinations plus "حسب الجهاز". It applies at once, without a restart (LANG-1), and it's stored with the settings, so a backup carries it (BAK-6). A shared recipe image uses the chosen accent on the light palette (SHARE-3, Decision 16).
- **LOOK-2** The accent changes only colour. It never changes a string, a tooltip, a shape, the order or position of a control, or what a screen shows, so muscle memory, screenshots in help and the widget tests hold in both.
- **LOOK-3** Contrast, in both accents and both brightnesses:
  - body text at least 4.5:1 and large text at least 3:1 against every surface it can sit on. Text over a photo sits on a scrim that guarantees it (a gradient to at least 78% near-black under a card's title) or on an opaque chip;
  - the boundary of a control — a text field, a chip, a checkbox, a switch, an outlined button — at least 3:1 against what surrounds it. A card is known by its content and a shadow and needs no 3:1 edge; in dark, where a shadow doesn't show, a card takes a 1 dp hairline;
  - disabled text at least 3:1, never Material's default 38% alpha;
  - hint text at full `onSurfaceVariant`, never faded.
  A pure-Dart test computes the WCAG 2.1 ratio of every text/surface pair each theme uses and fails below the bar.
- **LOOK-4** Amounts are the loudest text in an ingredient line (the app's claim is that they're right): the amount in the accent colour at weight 700, the rest of the line in the body colour at 400. The amount and the unit word that agrees with it stay one phrase (QTY-6): the line is split at the isolate characters `formatLine` already emits, never re-parsed. Cook mode's ingredient sheet and the grocery list do the same.
- **LOOK-5** Type: one family, IBM Plex Sans Arabic (400–700), letter-spacing 0 on every token, even leading. Line height 1.75 on any text that can wrap to a second line and 1.5 on single-line chrome — from the font's own metrics, 1.729 em clears any Arabic glyph pair, tashkeel included. Screen titles are 26/700 and the library's question 30/700; the full scale is in `docs/research/design-styles.md`.
- **LOOK-6** Shape and depth:
  - cards 22 dp, photo cards 24 dp, a sheet's top corners 28 dp;
  - buttons, chips, the search field and segmented controls are pills, and icon buttons are circles;
  - in light, cards, the search field and floating buttons carry one soft warm shadow, and the navigation pill and primary buttons a deeper one. Dark has no shadows. Material's elevation tint is never used.
  Decoration is drawn, never a bitmap (LOOK-10), and mirrors with the language through `EdgeInsetsDirectional` / `BorderDirectional`. No new package or font.
- **LOOK-7** Navigation is a floating pill at the bottom of the four main screens: الوصفات، الخطة، المشتريات، الإعدادات, with a raised round "+" in its centre (tooltip "أضف وصفة") that opens the add sheet (LOOK-11). Each slot has an icon and a label, and the current one is marked by colour **and** a dot, never by colour alone. The pill never overflows: at 1.3× text on a 360 dp phone its labels shrink to fit rather than wrap or clip. Pushed screens have no pill: a recipe, cook mode, the editor, import, a cookbook, the purchase screen.
- **LOOK-8** Every main screen renders in both accents, both brightnesses and both languages at 1.3× text on a 360 dp phone without overflow (LANG-6). Cook mode keeps COOK-2's sizes. The banner slot (ADS-9) sits above the navigation pill, or on the recipe page under its action bar, 8 dp from it and outside the scrolling content (ADS-3). An empty slot takes no space.
...
- **LOOK-10** A recipe with no photo gets a drawn cover wherever a photo would go: a library card, a list thumbnail, a plan entry, the recipe page's header. The cover is one of six tints, picked from the recipe's ID so it's the same on every screen and every device, with an eight-point-star line pattern in the tint's dark tone and the title's first letter large in the centre. It's never a grey block or a stock icon. A recipe page without a photo opens with a short cover header instead of the photo hero.
- **LOOK-11** The add sheet, from the centre "+", offers four ways in, in this order: from a link (IMP-1), from a photo (IMP-10), pasted text (IMP-3), and writing it yourself (the editor). Below them it shows this month's AI imports left with the reset date (IMP-7), and one line saying website imports are free and unlimited (IMP-2). It carries no upsell and no ad (ADS-1).
- **LOOK-12** The library home:
  - a time-of-day greeting and the question "ماذا نطبخ اليوم؟";
  - "كل الوصفات | كتب الطبخ" as a segmented control;
  - the search field (ORG-3), with a filter button that opens every ORG-6 filter, and a row of quick chips for the common filters and the most-used tags;
  - a "تابع الطبخ" card while cook mode can still resume (COOK-6);
  - then the recipes as photo cards (grid) or rows (list), whichever was chosen last (ORG-5).
- **LOOK-13** The recipe page:
  - the photo full-bleed at the top, with round floating buttons for back, share, edit and more;
  - the content on a sheet that overlaps the photo;
  - the facts that exist (prep, cook, servings: REC-3) as up to three tiles, and "في الخطة" when it's planned (PLAN-3);
  - two tabs, "المكونات | طريقة التحضير", opening on Ingredients. The scale and unit controls sit in the Ingredients tab, and the notes follow the steps;
  - an action bar at the bottom edge: "ابدأ الطبخ" (COOK-1), add to plan (PLAN-3) and add to groceries (GRO-2).
- **LOOK-14** Cook mode follows the light/dark setting like every other screen:
  - a segmented step rail;
  - the step at COOK-2's size, with its timers as large buttons (COOK-4) and a running timer as a band;
  - "السابق", the ingredients and "التالي" as three large controls at the bottom edge, where a wet thumb reaches them.
...
14. (20 September 2026; its design replaced by Decision 23 on 26 September 2026, which keeps the two names as accent colours) The app ships **two looks**, picked in Settings, not one: **حبر / Ink** (cream paper, near-black ink, a deep teal that marks every amount, hairlines instead of boxes, no shadows) and **زعفران / Saffron** (quiet cream page, solid saffron action blocks with near-black text on them, one corner cut at 45°). Each has a hand-tuned light and dark theme. Both keep every existing string, add no package, font or bitmap, and must pass AA for text on every surface in both brightnesses. Ink is the default. The full specification — palettes, type scale, components, per-screen plans — is in `docs/research/design-styles.md`; the rules get IDs with `/spec look` before any of it is built (Phase 3).
...
23. (26 September 2026) The owner judged the shipped looks too basic and asked for a full redesign with the same features. The app becomes **سُفرة / Sufra**:
    - a warm linen page with white cards and soft shadows;
    - big photo cards, and a recipe page under a full-bleed photo;
    - pill-shaped controls;
    - a dark floating navigation pill with a centre "+" that gathers every way to add a recipe;
    - drawn covers for recipes without a photo.

    It reverses three parts of Decision 14: Ink's "no shadow anywhere", its "no stadium shape", and the rule that a look may never change a layout. Now there is one layout, and the look is an accent colour (LOOK-1, LOOK-2). The owner took all four recommendations:
    - **حبر and زعفران stay, as the two accents.** Teal and paprika share one layout, so a stored choice and every backup carry over with no data change, and زعفران becomes the default for new installs.
    - **The recipe page splits into tabs**, Ingredients and Steps (LOOK-13).
    - **The plan shows one day at a time under a week strip** (PLAN-1).
    - **The rule changes above** (LOOK-3, LOOK-6–LOOK-8).

    The research: recipe apps on Dribbble and Behance, taken as ideas and never as copies.

    The spec, palettes and per-screen plans are in `docs/research/design-styles.md`, and the reference mockups are in `docs/design/sufra/`. It ships in five PRs, each merged in order:
    1. the foundation, navigation and the add sheet;
    2. the library and the import screens;
    3. the recipe page and cook mode;
    4. the plan and groceries;
    5. settings, the first run and the purchase screen.
