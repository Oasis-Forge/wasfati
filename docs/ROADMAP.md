...
- [ ] **Share-sheet import** (IMP-1, IMP-3–IMP-9; app side shipped this branch): a link or shared text goes to AI import when the device-only path (IMP-2) can't read it → progress, with the cost line before sending (IMP-3) → **preview to edit before saving** → save. Still open: "Report a mistake" (sends only the source link and the user's note, only when tapped) and the emulator-verified TikTok/Instagram/YouTube/Facebook share payloads below.
- [ ] **Instagram fallback** (IMP-12, Decision 8; paste half shipped this branch): when a caption can't be read, the app offers to paste it, read from the clipboard only on tap, with the original link kept as the source. The screenshot/photo half is Photo import, below.
